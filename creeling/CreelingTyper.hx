...
class CreelingTyper {
            ...
            switch( node.kind ){
                ...
                case NodeVarAccess:
                    ...
                    if( track != null && track.state = Owned ){
                        outputInstruction.push( "READ_OWNED var_ " + node.varID );
                    } else { 
                        outputInstruction.push( "READ_BORROW var_ " + node.varID );
                    }
                case NodeCall: 
                    var track = context.variables.get( node.varID );
                ...
                case NodeBlock:
                    if( node.children != null ){
                        outputInstructions = outputInstructions.concat(specializAndCheck(node.children));
                    }
                case NodeIfElse:
                    ...
                    outputInstrunction.push( "BRANCH_ELSE" );
    ...
    private function mergeBranchUnification( thenCtx: FlowContext, elseCtx: FlowContext ): FlowContext {
        ...
        for( id in thenCtx.variable.keys() ){
            ...
            if( thenTrack.state = Leaked || elseTrack.state == Leaked ) {
                merged.variables.set( id: id, type: thenTrack.type, state: Leaked );
            } else if ( thenTrack.state == Owned && elseTrack.state == Owned ){
                merged.variables.set( id: id, type: thenTrack.type, state: Owned );
            } else {
                merged.variables.set( id: id, type: thenTrack.type, state: Leaked );
            }
        }
